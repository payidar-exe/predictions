...
def predict_single_race():
    print(f"🔮 Analyzing {TARGET_CITY} Race {TARGET_RACE_NO} ({TARGET_DATE})...")
    ...
    # Fetch Specific Race ID for Antalya, Race 2
    pr_query = f"""
    ...
    full_city_name = pr_df.iloc[0]['city']
    
    # Fetch Entries
    entries_query = f"""
        SELECT pe.*, pr.race_no, pr.date, pr.city, pr.distance, pr.track_type, pr.race_type
    ...
    for i, row in results.iterrows():
        # AI Note Logic
        note = ""
        if row['momentum_5'] > 0.7: note = "Form Zirve"
        elif row['galop_score'] > 0.7: note = "İdman Harika"
        elif row['track_win_rate'] > 0.3: note = "Pisti Sever"
        elif row['score'] > 0.8: note = "Favori"
        else: note = "Sürpriz"
        
        print(f"{row['horse_name']:<20} | Skor: {row['score']:.2f} | {note}")
